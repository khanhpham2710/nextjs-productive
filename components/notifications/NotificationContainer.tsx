...
export default function NotificationContainer({ userId }: Props) {
  ...
  const { mutate: clearAllNotifications } = useMutation({
    ...
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: ["getUserNotifications"] });
      queryClient.setQueryData(["getUserNotifications"], []);
    ...
    mutationKey: ["clearAllNotifications"],
  });
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <HoverCard openDelay={250} closeDelay={250}>
        ...
        >
          ...
          {userNotifications && userNotifications?.length > 0 && <div className="w-full flex justify-end">
            <Button
              disabled={!isAnyClickedFalse}
              onClick={() => {
